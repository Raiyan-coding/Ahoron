...
async function loadExamPaper(year, monthIndex, day, subject, examStartUtc, examEndUtc, durationMinutes){
  ...
  if(_testModeActive && _testPaperId){
    let found = false;
    // Support testing for E1 (boards) and E2 (slots) even when _testPaperId is set
    if(subject.id === 'e1' && Array.isArray(data.boards) && data.boards.length > 0){
  ...
  if(Array.isArray(data.papers) && data.papers.length > 0){
    ...
    for (let si = 0; si < slots.length; si++) {
      ...
      const idx = RANDOMIZE_SET ? Math.floor(Math.random()*candidates.length) : pickPaperIndex(`${paperSeed}|slot${si}`, candidates.length);
      const q = candidates[idx];
      const norm = {
        id: q.id || `e2-s${si+1}-q${idx+1}`,
        text: q.text || q.q || '',
...
async function submitAnswers({auto=false} = {}){
    ...
    try{
      ...
      const payload = {
        ...
        answers,
        alias: paperAlias,
        manual: (subject.id === 'e1' || subject.id === 'e2') ? true : false,
      ...
      const contentObj = {
        ...
        manual: payload.manual ? true : false,
...
function showCongratsOverlay(contentObj){
  if(congratsOverlay){
    // Update message for manual-scored exams
    const msgEl = congratsOverlay.querySelector('.muted');
    if(contentObj && contentObj.manual){
      if(msgEl) msgEl.textContent = 'Your answers have been saved. This exam is graded manually — results will be added to the Results page by an instructor.';
    } else {
      if(msgEl) msgEl.textContent = 'Your answers have been saved.';
    }
...
function initScrollAnimations(){
  try{
    ...
    const selector = ['.card', '.q-card', '.exam-header', '.schedule-list li', '.exam-item', '.rank-row', '.summary-card'].join(', ');
    const nodes = Array.from(document.querySelectorAll(selector));
    if(!nodes.length) return;
    nodes.forEach((el,i)=>{
      // attach reveal class and stagger
